...
namespace Task
{
    class Program
    {
        ...
        public static List<int> _winingIndex = new List<int>(); 
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < _drawn.Count; i++)
            {
                ...
                foreach (var board in _boards)
                {
                    ...
                    if (isWinner)
                    {
                        var indexOfWinner = _boards.IndexOf(board);
                        if (!_winingIndex.Contains(indexOfWinner))
                        {
                            _winingIndex.Add(indexOfWinner);
                            /*Zadnji winner*/
                            if (_winingIndex.Count == _boards.Count)
                            {
